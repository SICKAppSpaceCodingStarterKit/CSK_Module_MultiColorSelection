-- Block namespace
local BLOCK_NAMESPACE = 'MultiColorSelection_FC.Process'
...
local function process(handle, imageSource)
  ...
  while true do
    local amount = CSK_MultiColorSelection.getInstancesAmount()
    if amount < instance then
      CSK_MultiColorSelection.addInstance()
    else
      CSK_MultiColorSelection.setInstance(instance)
      CSK_MultiColorSelection.setRegisterEvent(imageSource)
      break
    end
  end
  ...
  if mode == 'TOTAL_SUBRESULTS' then
    return 'CSK_MultiColorSelection.OnNewStringResult' .. tostring(instance)
  else
    return 'CSK_MultiColorSelection.OnNewResult' .. tostring(instance)
  end
end
